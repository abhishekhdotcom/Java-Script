...
export default async (req, res, next) => {
  ...
  try {
    ...
    const currentSessionData = await findSessionById(decoded.sessionId);
    user.lastLogin = currentSessionData.createdAt; // set LastLogin in req.user
    ...
    if (refreshToken) {
      try {
        ...
        const { session } = await authenticateUser({ user, req, res });

        // Find current Session by SessionId
        const currentSessionData = await findSessionById(session.sessionId);
        user.lastLogin = currentSessionData.createdAt; // set LastLogin in req.user
